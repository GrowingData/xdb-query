...
namespace Mung.Core {
	public class SQLiteBulkInsert {
		private static Dictionary<Type, DbType> TYPE_MAP = GetTypeMapping();
		...
		public void Flush() {
			try {
				if (m_trans != null)
					m_trans.Commit();
			} catch (Exception ex) { throw new Exception("Could not commit transaction. See InnerException for more details", ex); } finally {
				if (m_trans != null)
					m_trans.Dispose();
		...
		public long WriteToServer(IDataReader reader) {
			...
			if (reader.Read()) {
				...
				for (var i = 0; i < reader.FieldCount; i++) {
					...
					var dbType = TYPE_MAP[type];
					var name = reader.GetName(i);
					AddParameter(name, dbType);
				}
		...
		public void Insert(object[] paramValues) {
			...
			if (m_counter == m_commitMax) {
				try {
					...
					MungLog.LogException("SQLiteBulkInsert.Insert", ex);
		...
		private static Dictionary<Type, DbType> GetTypeMapping() {
